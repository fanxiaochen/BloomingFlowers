...
static void MATRIX_TO_PETAL(const PointMatrix& point_matrix, Petal& petal)
{
    size_t point_size = point_matrix.rows();
    ...
    {
        osg::Vec3 point;
        point.x() = point_matrix(j,0);
        point.y() = point_matrix(j,1);
        point.z() = point_matrix(j,2);

        hard_ctrs->push_back(point);
    }
}

static PointMatrix PETAL_TO_MATRIX(Petal& petal)
{
    PointMatrix point_matrix;

    size_t point_size = petal.getHardCtrsIndex().size();
    point_matrix.resize(point_size, 3);
    ...
    for (size_t j = 0, j_end = hard_idx.size(); j < j_end; ++ j)
    {
        ...
        point_matrix(j, 0) = point.x();
        point_matrix(j, 1) = point.y();
        point_matrix(j, 2) = point.z();
    }
