...
TipFittingTerm::TipFittingTerm(int petal_id)
    :petal_id_(petal_id)
{
}

void TipFittingTerm::build()
{
    buildA();
    buildb();
}
...
void TipFittingTerm::projection()
{
    return;
}

void TipFittingTerm::update()
{
    return;
}

void TipFittingTerm::buildA()
{
    Solver::DeformPetal& deform_petal = Solver::deform_petals_[petal_id_];
    Solver::CovMatrix& cov_matrix = deform_petal._cov_matrix;
    Solver::CorresMatrix& corres_matrix = deform_petal._tip_corres;
    ...
    for (int i = 0; i < ver_num; ++i) 
    {
        ...
        wi_x += zero_correction(Solver::lambda_tip_fitting_*(2/cov_matrix.col(i)[0])*corres_matrix.row(i).sum());
        wi_y += zero_correction(Solver::lambda_tip_fitting_*(2/cov_matrix.col(i)[1])*corres_matrix.row(i).sum());
        wi_z += zero_correction(Solver::lambda_tip_fitting_*(2/cov_matrix.col(i)[2])*corres_matrix.row(i).sum());
...
void TipFittingTerm::buildb()
{
    Solver::DeformPetal& deform_petal = Solver::deform_petals_[petal_id_];
    Solver::PetalMatrix& origin_petal = deform_petal._origin_petal;
    Solver::CloudMatrix& cloud_matrix = deform_petal._tip_cloud;
    Solver::CorresMatrix& corres_matrix = deform_petal._tip_corres;
    Solver::CovMatrix& cov_matrix = deform_petal._cov_matrix;
    Solver::ConvertAffineMatrix& convert_affine = deform_petal._convert_affine;
    ...
    for (size_t i = 0; i < ver_num; ++ i)
    {
        ...
        b_.col(i) = Solver::lambda_tip_fitting_*2*cov_matrix.col(i).asDiagonal().inverse()*weight_cloud;
    }
...
double TipFittingTerm::zero_correction(double value)
{
